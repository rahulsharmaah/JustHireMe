...
**Batch status:** P0.1-P0.4 implementation pass completed on 2026-05-07. Frontend build and backend API regression suite passed; a full manual browser smoke remains useful before release packaging.
...
**Status:** Implemented client-side save blocking for missing provider credentials/URLs, per-step warnings for missing override keys/models, reset-to-global controls, and settings save toasts.
...
**Status:** Implemented in the approval drawer using existing per-lead events. New lead saves record discovery, and the drawer shows scoring, generation, contact lookup, feedback, follow-up, status, and submission activity with timestamps.
